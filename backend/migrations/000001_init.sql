-- 000001_init.sql
-- Initial schema migration extracted from backend/db/schema.sql
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'pages_tsv_trigger') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'blog_posts_tsv_trigger') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'contact_submissions_tsv_trigger') THEN
